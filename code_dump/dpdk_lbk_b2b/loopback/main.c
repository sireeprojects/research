...
#define SOCKET_PATH "/tmp/vhost-user0"
...
int main(int argc, char **argv) {
    ...
    /* 2. Create Mempool */
    mbuf_pool = rte_pktmbuf_pool_create("MBUF_POOL", NUM_MBUFS, 250, 0, 
                                       RTE_MBUF_DEFAULT_BUF_SIZE, rte_socket_id());
    if (!mbuf_pool) 
        rte_exit(EXIT_FAILURE, "Mempool creation failed\n");

    /* 3. Create SPSC Ring */
    ring = rte_ring_create("SPSC_VHOST", RING_SIZE, rte_socket_id(), 
    ...
    if (rte_vhost_driver_register(SOCKET_PATH, RTE_VHOST_USER_CLIENT) < 0) 
        rte_exit(EXIT_FAILURE, "vHost register failed\n");

    if (rte_vhost_driver_callback_register(SOCKET_PATH, &vhost_ops) < 0)
        rte_exit(EXIT_FAILURE, "vHost callback failed\n");
