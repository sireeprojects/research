...
// Consumer thread dequeues from the ring and enqueues into guest RX virtqueue.
// Both paths are lossless — they retry with backpressure rather than dropping.
...
protected:
    // Override to transform packets between dequeue and enqueue.
    // Default implementation is a no-op pass-through (pure loopback).
    virtual void on_packet(struct rte_mbuf* /*pkt*/) {}
...
private:
    // Thread entry points.
    void producer_loop();   // dequeue guest TX → rte_ring
    void consumer_loop();   // rte_ring → enqueue guest RX
    ...
    static VhostController* instance_;

    // Per-thread stats (only touched by owning thread — no atomics needed).
    uint64_t rx_pkts_  = 0;   // producer: dequeued from guest
    uint64_t tx_pkts_  = 0;   // consumer: enqueued to guest
