...
void CmdlineParser::add_argument(std::string_view long_n, std::string_view short_n, 
                                 std::string_view desc, bool req, std::string_view default_v) {
    m_arguments.push_back({std::string(long_n), std::string(short_n), 
...
void CmdlineParser::parse(int argc, char** argv) {
    ...
    for (int i = start_index; i < argc; ++i) {
        ...
        if (!matched) throw std::runtime_error("Unknown argument '" + std::string(token) + "'");
    }
